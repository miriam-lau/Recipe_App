...
def initialize_app():
    ...
    recipe_manager = RecipeManager.create_and_initialize_recipe_manager()
    entry_manager = EntryManager.create_and_initialize_entry_manager()
...
def render_cookbook(id: int):
    ...
    recipes = recipe_manager.get_recipes_for_cookbook(id)
    return render_template(
...
def render_recipe(id: int):
    ...
    entries = entry_manager.get_entries_for_recipe(id)
    return render_template(
        'recipe.html',**locals())
...
def add_entry():
    ...
    entry_manager.add_new_entry(entry)
