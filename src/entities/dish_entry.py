...
class DishEntry(Entity):
    ...
    @staticmethod
    def from_dict(entity_id, parent, data) -> DishEntry:
        return DishEntry(entity_id, parent, data)

    def to_dict(self):
        return {
            Entity.ENTITY_ID_HEADER: str(self.entity_id),
            Entity.PARENT_ID_HEADER: str(self.parent.entity_id),
            DishEntry.DATE_HEADER: self.date_string(),
            DishEntry.MIRIAM_RATING_HEADER: str(self.miriam_rating),
            DishEntry.JAMES_RATING_HEADER: str(self.james_rating),
            DishEntry.MIRIAM_COMMENTS_HEADER: self.miriam_comments,
            DishEntry.JAMES_COMMENTS_HEADER: self.james_comments
        }

    def modify(self, data):
        if DishEntry.DATE_HEADER in data:
            self.date = datetime.datetime.strptime(data[DishEntry.DATE_HEADER], '%Y-%m-%d')
        if DishEntry.MIRIAM_RATING_HEADER in data:
            self.miriam_rating = float(data[DishEntry.MIRIAM_RATING_HEADER])
        if DishEntry.JAMES_RATING_HEADER in data:
            self.james_rating = float(data[DishEntry.JAMES_RATING_HEADER])
        if DishEntry.MIRIAM_COMMENTS_HEADER in data:
            self.miriam_comments = data[DishEntry.MIRIAM_COMMENTS_HEADER]
        if DishEntry.JAMES_COMMENTS_HEADER in data:
            self.james_comments = data[DishEntry.JAMES_COMMENTS_HEADER]

    @staticmethod
    def file_headers():
        return [Entity.ENTITY_ID_HEADER, Entity.PARENT_ID_HEADER, DishEntry.DATE_HEADER, DishEntry.MIRIAM_RATING_HEADER,
                DishEntry.JAMES_RATING_HEADER, DishEntry.MIRIAM_COMMENTS_HEADER, DishEntry.JAMES_COMMENTS_HEADER]
